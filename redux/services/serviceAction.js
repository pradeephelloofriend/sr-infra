...
export const setWorksCatData = data => ({
    type: serviceActionTypes.SET_WORKS_CAT_DATA,
    payload: data
...
export const fetchWorksCatData= () => {
    return async dispatch => {
        const res = await fetch(`${process.env.API_PATH}/work_category`)
        const data = await res.json()
        dispatch(setWorksCatData(data))   
